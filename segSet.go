...
// func (ss *segSet) add(from, to uint32, touchesEdges int) {
// 	if from == to {
...
// func (ss *segSet) findSegTo(to uint32, loop *loop) *seg {
...
// 	//panic("seg not found")
// }
// func (ss *segSet) findSegFrom(from uint32, loop *loop) *seg {
// 	for _, s := range ss.segs {
...
// func (ss *segSet) addFromEdgePens(tm *landMesh, vt, vn uint32, edgePens []uint32, cornerIsCut bool, output *landMesh) { //collect alternating segments))
...
// func (ss *segSet) otherEnd(v uint32) uint32 {
// 	for _, s := range ss.segs {
// 		if s.from == v {
// 			return s.to
// 		}
...
// func (ss *segSet) getEdgePenetrations(m *landMesh, vThis, vNext uint32) []uint32 {

// 	dm := map[float64]uint32{}
...
// func (ss *segSet) unUsedVert(loops *loopSet) uint32 {
...
// func (ss *segSet) findLinked(vi uint32, l *loop) uint32 {
...
// func (ss *segSet) getLoops(m *landMesh) *loopSet {
